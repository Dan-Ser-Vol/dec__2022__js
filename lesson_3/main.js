...
for (let i = 0; i < listOfItems.length; i++) {
    let liItem = listOfItems[i]
    document.write(`<li>- ${liItem} -</li>`)
}
document.write(`</ul>`)
...
for (let i = 0; i < users.length; i++) {
    const user = users[i];
    if (user.status) {
        document.write(`<h3 class="cl_true"> користувачі зі статусом true: ${user.name}</h3>`)
    }
}
document.write(`</div>`)
...
for (let i = 0; i < users.length; i++) {
    ...
    if (!user.status)
        document.write(`
<div class="product-card">
     <h3 class="cl_false"> користувачі зі статусом false: ${user.name}</h3>
</div>
`)
}
...
for (let i = 0; i < users.length; i++) {
    ...
    if (user.age > 30)
        document.write(`
<div class="product-card">
     <h3 class="old">  користувачі які старші за 30: ${user.name}, має років ${user.age}</h3>
</div>
`)
}
